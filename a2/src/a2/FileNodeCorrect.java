...
public class FileNode {
	 ...
	 * @return the parent
	 */
//Me: Are getters and setters necessary? 
	 ...
	 * be a directory
	 */
//Note: Label purpose of block code instead of what parts of the code does
...
//Note: JavaDoc, would be best to add parameters line(178 to 181)
	/**
...
//Note: I like this Note line(184 to 187), it is good to know what the proper input should be and that there is Exception if wrong input
		/*
		 * if (file.isDirectory()){ //This code won't work on directories. throw
		 * new UnexpectedFileTypeException("This is a directory"); }
		 */
//Note: use better names for variables. Something that makes it clear what it represents
		int j = file.getName().length() - 1;
		int k = 0;
//Note: not necessary to say why you are doing what you are doing, just do it and say what it does. also it would have been better if there were quotes around  
//period to make it clear ex: "."
		String extension = "."; // All filename extensions begin with a .
		boolean stop = false;
//Note: Strange method, good to leave note for oneself, suggest adding said method before final submission
		// Does java have a "break" option? I'm not sure.
		// Regardless, stop basically functions like break.
//Note: good that this part is well documented, suggest changing documentation to say what the purpose of this code is as opposed to what the code actually does
		while (k < file.getName().length() - 1 && !stop) {
			...
			if (file.getName().charAt(j) != '.') {
				...
				// should begin with a '.'
//Me: This is an odd implementation, look into further to see if it is good or bad
				k = k + 1;
